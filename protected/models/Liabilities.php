 ...
 * @property integer $id
 * @property string $liability_name
...
class Liabilities extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('liability_name', 'required'),
			array('amount', 'numerical'),
			array('liability_name', 'length', 'max'=>60),
			array('description', 'length', 'max'=>80),
			array('status', 'length', 'max'=>1),
			array('created_date', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, liability_name, description, amount, status, created_date', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('liability_name',$this->liability_name,true);
